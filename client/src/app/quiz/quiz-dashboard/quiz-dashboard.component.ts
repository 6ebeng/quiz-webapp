...
export class QuizDashboardComponent implements OnInit, OnDestroy {
	...
	getQuestionsCount(id: string): Observable<number> {
		return this.quizService.assignedQuestionsSubject.pipe(map((questions) => (questions ? questions.filter((question) => question.quizId === id).length : 0)));
	}

	getCategoryColor(id: string): Observable<string> {
		return this.quizService.getQuizCategory(id).pipe(map((category) => (category?.color ? category.color : 'silver')));
	}
