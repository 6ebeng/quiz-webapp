...
export class AdminQuestionsComponent implements OnInit, OnDestroy {
	...
	openAiImg: string = '/assets/img/' + 'openai.png';
	...
	ngOnInit() {
		this.questionsSubscription = this.quizService.getAllQuestions().subscribe((quizQuestions) => {
			this.questions = quizQuestions;
		});

		this.assignedQuestionsSubscription = this.quizService.getAssignedQuestions().subscribe((assignedQuestions) => {
			this.assignedQuestions = assignedQuestions;
		});

		this.categoriesSubscription = this.quizService.getQuizCategories().subscribe((categories) => {
	...
	initForm() {
		this.optionsArray = new FormArray([new FormControl('', Validators.required), new FormControl('', Validators.required), new FormControl('', Validators.required)]);
	...
	isQuestionAssigned(id: string): Observable<boolean> {
		return this.quizService.getAssignedQuestions().pipe(map((asquestions) => asquestions.some((asquestion) => asquestion.questionId === id)));
	}
	...
	getQuestionTooltip(id: string) {
		...
		return names ? 'Unable to delete. First remove from ' + (quizzes.length > 1 ? 'quizzes: ' : 'quiz: ') + names : '';
	}
	...
	generateQuestion() {
		...
		this.dataService.generateQuestion({ category: this.aiCategory, answers: this.aiAnswers.toString() }).subscribe({
