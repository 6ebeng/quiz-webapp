...
export class AdminQuizzesComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
		...
		this.quizQuestionsSubscription = this.quizService.getAllQuestions().subscribe((quizQuestions) => {
	...
	initForm() {
		if (this.mode == 'add') {
			this.questionsArray = new FormArray([new FormControl('', Validators.required), new FormControl('', Validators.required), new FormControl('', Validators.required)]);
		} else {
	...
	updateForm(): void {
		...
		const aQuestionIds = this.assignedQuestions.filter((aq) => aq.quizId === this.selectedQuiz?.id).map((aq) => aq.questionId);

		const quizQuestions = this.quizQuestions.filter((question) => aQuestionIds.includes(question.id));

		quizQuestions.forEach((question) => this.questionsArray.push(new FormControl(question.text, Validators.required)));
				...
				next: (status: string) => {
					if (status === 'OK') {
						...
						asQuestions.forEach((asQuestion) => {
							if (!questionsIds.some((id: string) => id === asQuestion.questionId)) this.quizService.deleteAssignedQuestion(asQuestion.id);
						});
	...
	getCategoryImage(id: string): Observable<string> {
		return this.quizService.getQuizCategory(id).pipe(
			map((category) => {
