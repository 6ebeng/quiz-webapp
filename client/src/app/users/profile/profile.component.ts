...
export class ProfileComponent {
	...
	onEdit() {
		if (this.edit) {
			let user = { ...this.profileForm.value, id: this.user.id, username: this.user.username, password: this.user.password, role: this.user.role, imageUrl: this.user.imageUrl };
	...
	onFileSelected(event: any) {
		...
		if (file && file.size < 200 * 1024) {
			...
			this.authService.editProfileImg(this.user.id, file).subscribe({
				next: (status: string) => {
					if (status === 'OK') {
	...
	removeImage() {
		this.authService.deleteProfileImg(this.user.id).subscribe({
			next: (status: string) => {
				console.log(status);
